...
class LibraryFragment : Fragment() {
    ...
    override fun onCreateView(inflater: LayoutInflater, container: ViewGroup?, savedInstanceState: Bundle?): View? {
        ...
        view.findViewById<ExtendedFloatingActionButton>(R.id.fabAddPlaylist).setOnClickListener { showCreatePlaylistDialog() }
        view.findViewById<ExtendedFloatingActionButton>(R.id.fabRename).setOnClickListener { showRenamePlaylistDialog() }
    ...
    private fun showCreatePlaylistDialog() {
        ...
        et.hint = "플레이리스트 이름"
        AlertDialog.Builder(context).setTitle("새 플레이리스트 추가").setView(et)
    ...
    private fun showRenamePlaylistDialog() {
        ...
        AlertDialog.Builder(context).setTitle("플레이리스트 이름 변경").setView(et)
    ...
    private fun refreshAll() {
        tabAdapter.refreshData() // 어댑터 내 함수명 변경 대응
        updateMergedList()
    }
    ...
    private fun updateMergedList() {
        ...
        rawSongs.forEach { s ->
            ...
            val brandKey = s.brand?.lowercase() ?: "unknown"
            merged.brandNumbers[brandKey] = s.no ?: ""
        }
